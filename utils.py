...
def addPosFeatureIP(features):
    ...
    for ind, var in enumerate(vn):
        ...
        a, b = int(ss[0]), int(ss[1])
        for d in range(D1):
            groupPos[ind,d] = PF(d,D1,int(b/nBin*15))#PF(d,D1,b)#
        for d in range(D2):
            elementPos[ind,d] = PF(d,D2,int(a/nItem*170))#PF(d,D2,a)#
    features.groupFeatures = np.concatenate([ groupPos, elementPos], axis=-1)
    # features.varFeatures = np.concatenate([groupPos, elementPos], axis=-1)
...
#     D1 = 16
#     D2 = 16
#     groupPos = np.zeros((vf.shape[0], 1))
#     elementPos = np.zeros((vf.shape[0], 1))
#     for ind, var in enumerate(vn):
#         if 'place' not in var:
#             continue
#         ss = re.findall('\d+', var)
